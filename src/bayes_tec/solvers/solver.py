...
class Solver(object):
    ...
    def _train_dataset_iterator(self, data_shape,  sess=None, minibatch_size=128,seed=0, **kwargs):
        ...
        data = data.map(lambda indices: \
                tuple(tf.py_func(lambda indices: self._get_data(indices,data_shape),[indices], [settings.float_type]*self.model_input_num)),num_parallel_calls=12)#Y_var, freqs, X, Y
