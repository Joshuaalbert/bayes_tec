...
class MomentGeneratingFunctionImprovement(Acquisition):
    """
    ...
    def __init__(self, model, t=0.5):
        ...
        """
        super(MomentGeneratingFunctionImprovement, self).__init__(model)
        self.fmin = DataHolder(np.zeros(1))
        self.t = DataHolder(np.array(t))

    def _setup(self):
        super(MomentGeneratingFunctionImprovement, self)._setup()
    ...
    def _build_acquisition(self, Xcand):
        ...
        s = tf.sqrt(candidate_var)
        m = candidate_mean

        mp = candidate_mean - candidate_var * self.t

        # Compute the acquisition
        normal = tf.contrib.distributions.Normal(mp, s)
        t1 = normal.cdf(self.fmin)
        t2 = tf.exp((self.fmin - candidate_mean - 1.)*self.t + 0.5 * candidate_var * self.t)
        return tf.multiply(t1, t2, name=self.__class__.__name__)
